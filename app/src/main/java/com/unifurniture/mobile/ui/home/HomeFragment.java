...
public class HomeFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Tint search icon; hide SearchView's internal collapse/nav button (shows as ≡)
        ...
        // Tap the dim backdrop → dismiss search UI
        binding.searchDimBackdrop.setOnClickListener(v -> clearSearchUiState());
    ...
    private void observeData() {
        ...
        viewModel.getSearchSuggestions().observe(getViewLifecycleOwner(), products -> {
            if (products != null && !products.isEmpty()) {
                ...
                searchSuggestionAdapter.submitList(products.subList(0, limit));
    ...
    private void setupSearchSuggestions() {
        ...
        binding.rvSearchSuggestions.addItemDecoration(
    ...
    /** Hide the whole overlay if neither panel is visible. */
    private void maybeHideOverlay() {
        if (binding.rvSearchSuggestions.getVisibility() != View.VISIBLE
                && binding.layoutSearchHistory.getVisibility() != View.VISIBLE) {
            binding.searchOverlay.setVisibility(View.GONE);
        }
    }

    /** Collapse overlay, both panels, restore content. */
